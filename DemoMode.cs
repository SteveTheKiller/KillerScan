...
namespace KillerScan
{
    ...
    public partial class MainWindow
    {
        ...
        private static readonly string[] DemoSubnets =
            { "10.0.0", "10.10.10", "172.16.4", "192.168.0", "192.168.1", "192.168.50" };

        // Always the .1 gateway.
        private static readonly (string Vendor, string[] Ouis, string Type, int[] Ports, string[] Names) DemoGateway =
        ...
        private static readonly (string Vendor, string[] Ouis, string Type, int[] Ports, string[] Names)[] DemoPool =
        {
        ...
        {
            var rng = _demoRng;
            string net = DemoSubnets[rng.Next(DemoSubnets.Length)];

            SubnetInput.Text    = $"{net}.0/24";
            GatewayLabel.Text   = $"{net}.1";
            DnsLabel.Text       = $"{net}.1";
            LocalIpLabel.Text   = $"{net}.{rng.Next(20, 60)}";
            InterfaceLabel.Text = rng.Next(2) == 0 ? "Ethernet" : "Wi-Fi";
            _portableBadge.Visibility = System.Windows.Visibility.Collapsed;

            _active.Devices.Clear();
            var used = new HashSet<int> { 1 };
            _active.Devices.Add(MakeDemoDevice(net, 1, DemoGateway, rng));
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                _active.Devices.Add(MakeDemoDevice(net, host, DemoPool[rng.Next(DemoPool.Length)], rng));
            }
        ...
        private static NetworkDevice MakeDemoDevice(
            string net, int host,
